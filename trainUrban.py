...
FT_URBAN_ALL_CFG = {
    "train": {
        ...
        "test_data_path": "../urban/origin.mat",
        "log_dir": "saved_models/urban",
        "pretrained_path": "",
        "scene_id": 0,
        "alpha": 0.8,
    }
...
class UrbanTrain:
    def __init__(self, cfg_name):
        self.ft_cfg = FT_URBAN_ALL_CFG[cfg_name]
        ft_cfg = FT_URBAN_ALL_CFG[cfg_name]
    ...
    def finetune(self, model, train_loader):
        ...
        scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[15,], gamma=0.25, last_epoch=-1)
        ...
        for epoch in range(1, total_epoch + 1):
            ...
            print('[{}/{}] | loss: {:.2e} | lr: {:.2e} '
                  .format(epoch, total_epoch, train_loss, cur_lr), file=logger, flush=True)

            if epoch % 50 == 0 or epoch >= 196:
                save_train(model_save_dir, model, optimizer, epoch=epoch)

    def _load_data(self):
        train_data = scio.loadmat(self.ft_cfg['test_data_path'])
        noise_imgs = train_data['img_n']
        noise_imgs = np.transpose(noise_imgs, (2, 0, 1))

        self.hsi_shape = noise_imgs.shape  # store img shape (210 300 300)
